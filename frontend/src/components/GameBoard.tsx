        ...
        return (
            <div className="min-h-screen flex items-center justify-center p-4">
                <div className="glass-panel p-12">
                    <p className="text-slate-800">Loading game...</p>
                </div>
            </div>
        );
    }
        ...
        <div className="min-h-screen p-4">
            ...
            <div className="glass-panel p-3 mb-4 max-w-6xl mx-auto">
                <div className="flex justify-between items-center">
                    <h1 className="text-xl md:text-2xl font-bold text-slate-800">Chain Reaction</h1>
                    <div className="flex gap-2 items-center">
                        <div className="text-xs md:text-sm text-slate-500">Round: {gameState.roundNumber}</div>
            ...
            <div className="max-w-6xl mx-auto flex flex-col md:flex-row gap-4 md:gap-6">
                ...
                <div className="glass-panel p-3 md:p-4 w-full md:w-64 flex flex-col order-2 md:order-1">
                    <h2 className="text-lg md:text-xl font-bold mb-2 md:mb-4 text-slate-800">Players</h2>
                            ...
                            >
                                <div className="flex items-center gap-2 justify-between">
                                    <div className="flex items-center gap-2">
                                        ...
                                        />
                                        <span className="font-semibold text-sm md:text-base text-slate-800 truncate max-w-[80px] md:max-w-none">{player.name}</span>
                                    </div>
                                    <span className="text-xs text-slate-500">{player.orbCount}</span>
                                </div>
                    ...
                    {gameState.isGameOver && gameState.winnerId && (
                        <div className="mt-4 md:mt-6 glass-panel p-3 md:p-4 bg-indigo-100 border-2 border-indigo-500">
                            <div className="text-base md:text-lg font-bold text-slate-800 text-center">
                                🎉 Game Over!
                            </div>
                            <div className="text-xs md:text-sm text-slate-600 text-center mt-1 md:mt-2">
                ...
                <div className="flex-1 glass-panel p-2 md:p-4 order-1 md:order-2">
                    <div className="text-center mb-2 md:mb-4">
                        <div className="text-sm md:text-lg text-slate-600">Current Turn:</div>
                        ...
                        {myTurn && !gameState.isGameOver && (
                            <div className="text-xs md:text-sm text-green-600 mt-1 md:mt-2">
                                ✓ Your turn!
                            </div>
                        )}
                        {!myTurn && !gameState.isGameOver && (
